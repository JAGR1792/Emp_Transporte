...
import { Menu, X, Truck, MapPin, Phone, Mail, Users, Facebook, Twitter, Instagram, Linkedin } from 'lucide-react'
    ...
    >
      <nav className="container-page" aria-label="Navegación principal">
        <div className="flex items-center justify-between h-14 lg:h-18">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2 z-50 min-w-0" aria-label="Transporte Nacional - Inicio">
            <div className="relative flex-shrink-0">
              <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-radius-xl bg-gradient-to-br from-brand-600 to-brand-400 flex items-center justify-center">
                <Truck className="w-5 h-5 lg:w-6 lg:h-6 text-white" />
              </div>
              <motion.span
                className="absolute -bottom-1 -right-1 w-2.5 h-2.5 lg:w-3 lg:h-3 bg-green-500 rounded-full border-2 border-white"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ delay: 0.3, type: 'spring', stiffness: 260, damping: 20 }}
              />
            </div>
            <span className="font-display font-bold text-heading-md lg:text-heading-lg text-slate-900 hidden sm:block truncate">
              Transporte Nacional
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {NAV_ITEMS.map((item) => (
              <Link
                ...
                to={item.path}
                className={cn(
                  'px-3 lg:px-4 py-2 text-body-sm lg:text-body-md font-medium rounded-radius-lg transition-all duration-fast whitespace-nowrap',
          ...
          <div className="hidden lg:flex items-center gap-2 lg:gap-3">
            <Link to="/rastreo">
              <Button variant="ghost" size="sm" leftIcon={<MapPin className="w-3.5 h-3.5 lg:w-4 lg:h-4" />}>
                Rastrear
              </Button>
            </Link>
            <Link to="/login">
              <Button variant="outline" size="sm" leftIcon={<Users className="w-3.5 h-3.5 lg:w-4 lg:h-4" />}>
                Ingresar
              </Button>
            </Link>
            <Link to="/pasajeros">
              <Button size="sm" leftIcon={<Truck className="w-3.5 h-3.5 lg:w-4 lg:h-4" />}>
                Comprar tiquetes
              </Button>
            </Link>
          </div>
          ...
          {/* Mobile Menu Button */}
          <button
            className="lg:hidden p-2 rounded-radius-lg text-slate-600 hover:bg-slate-100 transition-colors flex-shrink-0"
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="py-4 space-y-1 px-2">
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-col gap-3 px-2">
  ...
  return (
    <footer className="bg-slate-950 text-slate-300" role="contentinfo">
      <div className="container-page py-12 lg:py-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 lg:gap-12">
          {/* Brand */}
        ...
        {/* Bottom Bar */}
        <div className="mt-10 lg:mt-16 pt-8 border-t border-slate-800">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-body-sm text-slate-500 text-center sm:text-left">
              © 2026 Transporte Nacional. Todos los derechos reservados.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-body-sm text-slate-500">
...
export const MainLayout = ({ children }: { children: React.ReactNode }) => (
  <div className="min-h-screen flex flex-col overflow-x-hidden">
    <Header />
    <main className="flex-1 pt-14 lg:pt-18" id="main-content">
      {children}
    </main>
    <Footer />
  </div>
